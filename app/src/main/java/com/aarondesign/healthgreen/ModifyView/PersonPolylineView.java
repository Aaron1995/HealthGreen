...
/**
 * Created by Aaron on 2016/3/25 0025.
 */
public class PersonPolylineView extends View {
    ...
    public PersonPolylineView(Context context) {
        super(context);
        setWillNotDraw(false);
    }

    public PersonPolylineView(Context context, AttributeSet attr) {
        super(context, attr);
        datas = new ArrayList<>();
     ...
     * @param datas 总数据
     */
    private void getMaxValue(List<PersonBean> datas) {
        maxValue = 0;
        dataNum = 0;
        for (PersonBean person : datas) {
            if (Float.parseFloat(person.getInhale()) > maxValue) {
                maxValue = Float.parseFloat(person.getInhale());
            }
            dataNum++;
        }
    }
    ...
    private int startX = 150;//开始绘制的x坐标
    ...
    public void setDatas(List<PersonBean> datas) {
        this.datas = datas;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null != datas) {
            ...
            // 画折线
            for (PersonBean person : datas) {
                float value = Float.parseFloat(person.getInhale());
                if (isDrawFirst) {
                    //当第一次绘制时得到第一个点的横纵坐标
                    firstX = startX;
